...
def kMeansScore(data, allKs=[1], datasetType=None, target=None):
    ...
    ks = []
    kmScore = []
    f = open('plots/breastCancerPlots/KMeanClusterStats_'+datasetType+'.txt', 'w')
...
# Load data
dataSet = customDataset("BreastCancerWisconsinDataset.txt")
...
# NN for PCA
trainInputs = dataTrainScaledPCAoptimal
trainTarget = dataSet.targetTrain
testInputs = pcaOptimal.fit_transform(dataSet.dataTestScaled)
testTarget = dataSet.targetTest

print("\n\nBackprop NN With PCA Results: ")
epoch_v, trnerr_backprop, tsterr_backprop = NNBackPropCustom(trainInputs, trainTarget, testInputs, testTarget, inputDim=trainInputs.shape[1], targetDim=trainTarget.shape[1], numClass=np.unique(trainTarget).shape[0], classLabels=['Benign','Malignant'])
plot_learning_curve(epoch_v, trnerr_backprop, tsterr_backprop, "Backprop NN With PCA", "Epochs", "Error %", ylim=None)


# NN for PCA & K-Means
trainInputs = np.column_stack((dataTrainScaledPCAoptimal,dataTrainScaledKM))
trainTarget = dataSet.targetTrain
testInputs = np.column_stack((pcaOptimal.fit_transform(dataSet.dataTestScaled),dataTestScaledKM))
testTarget = dataSet.targetTest

print("\n\nBackprop NN With PCA & K-MEANs Results: ")
epoch_v, trnerr_backprop, tsterr_backprop = NNBackPropCustom(trainInputs, trainTarget, testInputs, testTarget, inputDim=trainInputs.shape[1], targetDim=trainTarget.shape[1], numClass=np.unique(trainTarget).shape[0], classLabels=['Benign','Malignant'])
plot_learning_curve(epoch_v, trnerr_backprop, tsterr_backprop, "Backprop NN With PCA and KMeans", "Epochs", "Error %", ylim=None)
...
for i in range(1,dataSet.dataTrainScaled.shape[1]+1,1):
    ...
    # NN for ICA
    trainInputs = dataTrainScaledICA
    trainTarget = dataSet.targetTrain
    testInputs = ica.fit_transform(dataSet.dataTestScaled)
    testTarget = dataSet.targetTest
    
    print("\n\nBackprop NN With ICA Results: "+str(i))
    epoch_v, trnerr_backprop, tsterr_backprop = NNBackPropCustom(trainInputs, trainTarget, testInputs, testTarget, inputDim=trainInputs.shape[1], targetDim=trainTarget.shape[1], numClass=np.unique(trainTarget).shape[0], classLabels=['Benign','Malignant'])
    plot_learning_curve(epoch_v, trnerr_backprop, tsterr_backprop, "Backprop NN With ICA "+str(i), "Epochs", "Error %", ylim=None)
    
    # NN for ICA & K-Means
    trainInputs = np.column_stack((dataTrainScaledICA,dataTrainScaledKM))
    trainTarget = dataSet.targetTrain
    testInputs = np.column_stack((ica.fit_transform(dataSet.dataTestScaled),dataTestScaledKM))
    testTarget = dataSet.targetTest
    
    print("\n\nBackprop NN With ICA & K-MEANs Results: "+str(i))
    epoch_v, trnerr_backprop, tsterr_backprop = NNBackPropCustom(trainInputs, trainTarget, testInputs, testTarget, inputDim=trainInputs.shape[1], targetDim=trainTarget.shape[1], numClass=np.unique(trainTarget).shape[0], classLabels=['Benign','Malignant'])
    plot_learning_curve(epoch_v, trnerr_backprop, tsterr_backprop, "Backprop NN With ICA and KMeans "+str(i), "Epochs", "Error %", ylim=None)
...
for i in range(1,dataSet.dataTrainScaled.shape[1]+1,1):
    ...
    # NN for RP
    trainInputs = dataTrainScaledRP
    trainTarget = dataSet.targetTrain
    testInputs = randomProj.fit_transform(dataSet.dataTestScaled)
    testTarget = dataSet.targetTest
    
    print("\n\nBackprop NN With RP Results: "+str(i))
    epoch_v, trnerr_backprop, tsterr_backprop = NNBackPropCustom(trainInputs, trainTarget, testInputs, testTarget, inputDim=trainInputs.shape[1], targetDim=trainTarget.shape[1], numClass=np.unique(trainTarget).shape[0], classLabels=['Benign','Malignant'])
    plot_learning_curve(epoch_v, trnerr_backprop, tsterr_backprop, "Backprop NN With RP "+str(i), "Epochs", "Error %", ylim=None)
    
    # NN for RP & K-Means
    trainInputs = np.column_stack((dataTrainScaledRP,dataTrainScaledKM))
    trainTarget = dataSet.targetTrain
    testInputs = np.column_stack((randomProj.fit_transform(dataSet.dataTestScaled),dataTestScaledKM))
    testTarget = dataSet.targetTest
    
    print("\n\nBackprop NN With RP & K-MEANs Results: "+str(i))
    epoch_v, trnerr_backprop, tsterr_backprop = NNBackPropCustom(trainInputs, trainTarget, testInputs, testTarget, inputDim=trainInputs.shape[1], targetDim=trainTarget.shape[1], numClass=np.unique(trainTarget).shape[0], classLabels=['Benign','Malignant'])
    plot_learning_curve(epoch_v, trnerr_backprop, tsterr_backprop, "Backprop NN With RP and KMeans "+str(i), "Epochs", "Error %", ylim=None)
...
# NN for Decision Tree Feature Reduction
trainInputs = dataTrainScaledFSDT
trainTarget = dataSet.targetTrain
testInputs = clf.transform(dataSet.dataTestScaled)
testTarget = dataSet.targetTest

print("\n\nBackprop NN With Decision Tree Feature Selection: ")
epoch_v, trnerr_backprop, tsterr_backprop = NNBackPropCustom(trainInputs, trainTarget, testInputs, testTarget, inputDim=trainInputs.shape[1], targetDim=trainTarget.shape[1], numClass=np.unique(trainTarget).shape[0], classLabels=['Benign','Malignant'])
plot_learning_curve(epoch_v, trnerr_backprop, tsterr_backprop, "Backprop NN With Decision Tree Feature Selection", "Epochs", "Error %", ylim=None)

# NN for RP & K-Means
trainInputs = np.column_stack((dataTrainScaledFSDT,dataTrainScaledKM))
trainTarget = dataSet.targetTrain
testInputs = np.column_stack((clf.transform(dataSet.dataTestScaled),dataTestScaledKM))
testTarget = dataSet.targetTest

print("\n\nBackprop NN With DT Feature Selection & K-MEANs Results: "+str(i))
epoch_v, trnerr_backprop, tsterr_backprop = NNBackPropCustom(trainInputs, trainTarget, testInputs, testTarget, inputDim=trainInputs.shape[1], targetDim=trainTarget.shape[1], numClass=np.unique(trainTarget).shape[0], classLabels=['Benign','Malignant'])
plot_learning_curve(epoch_v, trnerr_backprop, tsterr_backprop, "Backprop NN With DT Feature Selection and KMeans "+str(i), "Epochs", "Error %", ylim=None)


# NN for Original Data
trainInputs = dataSet.dataTrainScaled
trainTarget = dataSet.targetTrain
testInputs = dataSet.dataTestScaled
testTarget = dataSet.targetTest

print("\n\nBackprop NN With Original Data Results: ")
epoch_v, trnerr_backprop, tsterr_backprop = NNBackPropCustom(trainInputs, trainTarget, testInputs, testTarget, inputDim=trainInputs.shape[1], targetDim=trainTarget.shape[1], numClass=np.unique(trainTarget).shape[0], classLabels=['Benign','Malignant'])
plot_learning_curve(epoch_v, trnerr_backprop, tsterr_backprop, "Backprop NN With Original Data", "Epochs", "Error %", ylim=None)