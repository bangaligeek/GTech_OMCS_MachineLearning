...
def plot_curve(graphData, graph_title, graph_xlabel, graph_ylabel, ylim=None):
    ...
    plt.legend(loc="best")
    plt.savefig('plots/car/'+graph_title+'.png')
    plt.close()
    plt.show()
...
# K-MEANS
ks, kmScore = kMeansScore(dataSet.dataTrainScaled, range(1,20,5))
...
ksEM, lgLikelyhood, bicScore = EMscore(dataSet.dataTrainScaled, range(1,20,5))
...
pcaOptimal = PCA(n_components=15, whiten=False)
dataTrainScaledPCAoptimal = pcaOptimal.fit_transform(dataSet.dataTrainScaled)

# K-MEANS
ksPCAoptimal, kmScorePCAoptimal = kMeansScore(dataTrainScaledPCAoptimal, range(1,20,5))
...
# Expectation Maximization
ksEMPCA, lgLikelyhoodPCA, bicScorePCA = EMscore(dataTrainScaledPCAoptimal, range(1,20,5))
...
pca3Comp = PCA(n_components=16, whiten=False)
dataTrainScaledPCA3Comp = pca3Comp.fit_transform(dataSet.dataTrainScaled)

# K-MEANS
ksPCA3Comp, kmScorePCA3Comp = kMeansScore(dataTrainScaledPCA3Comp, range(1,50,5))
...
for i in range(1,dataSet.dataTrainScaled.shape[1]+1,1):
    ...
    # K-MEANS
    ksICA, kmScoreICA = kMeansScore(dataTrainScaledICA, range(1,20,5))
    ...
    # Expectation Maximization
    ksEMICA, lgLikelyhoodICA, bicScoreICA = EMscore(dataTrainScaledICA, range(1,20,5))
...
for i in range(1,dataSet.dataTrainScaled.shape[1]+1,1):
    ...
    # K-MEANS
    ksRP, kmScoreRP = kMeansScore(dataTrainScaledRP, range(1,50,5))
    ...
    # Expectation Maximization
    ksEMRP, lgLikelyhoodRP, bicScoreRP = EMscore(dataTrainScaledRP, range(1,20,5))
...
# K-MEANS
ksFTDT, kmScoreFTDT = kMeansScore(dataTrainScaledFSDT, range(1,50,5))
...
# Expectation Maximization
ksEMFTDT, lgLikelyhoodFTDT, bicScoreFTDT = EMscore(dataTrainScaledFSDT, range(1,20,5))
