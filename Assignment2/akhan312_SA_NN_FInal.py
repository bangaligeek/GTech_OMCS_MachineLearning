...
for s in stepSize:
    ...
    for i in xrange(epochs):
        ...
        epoch_v.append(i*k)
        trnerr_sa.append(winner['trainingError'])
        tsterr_sa.append(winner['testError'])
        print ("Winning Trainning Error: %5.2f%%" % winner['trainingError'], " Winning Test Error: %5.2f%%" % winner['testError'], " First Epoch For Lowest: %4d" %  winner['FirstEpoch'], "Last Epoch For Lowest: %4d" %  winner['LastEpoch'])

    ylim = (0, 70)
    xlim = (50, 1005)
    plot_learning_curve(epoch_v, trnerr_sa, tsterr_sa, "Neural Network With SA_step_"+str(s), "Epochs", "Error %", ylim, xlim=None)
