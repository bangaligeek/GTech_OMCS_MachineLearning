...
#fnn_rhc.params[:] = fnn_backprop.params[:]
...
for i in xrange(epochs):
    ...
    trnresult_newM = percentError(testOnClassData_custom(fnn_rhc, dataset=training_data), training_data['class'])
    tstresult_newM = percentError(testOnClassData_custom(fnn_rhc, dataset=test_data), test_data['class'])
    print ("epoch: %4d" %  i, " train error for negative step: %5.2f%%" % trnresult_newM, " test error for negative step: %5.2f%%" % tstresult_newM)

    best_step_error = min(trnresult_newP, trnresult_newM)

    if(best_step_error <= winner['trainingError']):
        if(best_step_error == trnresult_newP):
            if(best_step_error != winner['trainingError']):
                winner['FirstEpoch'] = i
            ...
            if(best_step_error != winner['trainingError']):
                winner['FirstEpoch'] = i
            winner['trainingError'] = trnresult_newM
            winner['testError'] = tstresult_newM
            winner['LastEpoch'] = i
            winner['w'] = NWM[:]
    '''
